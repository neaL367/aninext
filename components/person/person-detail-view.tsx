...
import { formatFuzzyDate, formatPersonName, formatValue } from "@/lib/anilist/display/format";
...
function getCreditRole(
  kind: PersonKind,
  edge: NonNullable<
    NonNullable<CharacterDetail["media"]>["edges"] | NonNullable<StaffDetail["staffMedia"]>["edges"]
  >[number],
) {
...
export function PersonDetailView({ kind, person }: PersonDetailViewProps) {
  ...
  const nativeName = person.name?.native && person.name.native !== name ? person.name.native : null;
      ...
      <div className={DETAIL_BODY_GRID_CLASS}>
        <aside className="flex flex-col gap-4 lg:sticky lg:top-20 lg:self-start">
          ...
          <DetailFactsPanel>
            ...
            <DetailFactRow label="Birthday" value={formatFuzzyDate(person.dateOfBirth)} />
            <DetailFactRow label="Blood type" value={formatValue(person.bloodType)} />
            <DetailFactRow
              label="Favourites"
              value={person.favourites != null ? String(person.favourites) : "—"}
            />
          </DetailFactsPanel>
        </aside>

        <div className="flex min-w-0 flex-col gap-8 lg:gap-10">
          <DetailEntityHeader entityLabel={entityLabel} title={name} subtitle={nativeName} />
