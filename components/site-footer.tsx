    ...
    <footer className="mt-16 border-t border-border-soft bg-surface-1/35 pb-20 md:pb-0">
      <div className="mx-auto grid w-full max-w-[1680px] gap-12 px-4 py-12 sm:px-7 lg:grid-cols-[1.5fr_1fr_1fr_1fr] lg:px-10 lg:py-16">
        ...
        <div>
          ...
          <p className="mt-4 text-sm leading-6 text-muted-foreground">Built for fast scans, deliberate choices, and fewer dead ends.</p>
          <Link href="/anime/top100" className="mt-5 inline-flex items-center gap-2 font-mono text-xs uppercase tracking-[0.12em] text-signal hover:underline">
      ...
      <div className="border-t border-border-soft">
        <div className="mx-auto flex w-full max-w-[1680px] items-center justify-between px-4 py-4 sm:px-7 lg:px-10">
          <p className="font-mono text-[0.62rem] uppercase tracking-[0.14em] text-muted-foreground">© 2026 AniNext</p>
          <p className="font-mono text-[0.62rem] uppercase tracking-[0.14em] text-muted-foreground">Not affiliated with AniList</p>
        </div>
      </div>
    </footer>
  );
...
function FooterColumn({ title, items }: { title: string; items: Array<{ href: Route; label: string }> }) {
  return (
    <div>
      ...
      <ul className="mt-4 space-y-3">
        {items.map((item) => (
          <li key={item.href}>
            <Link href={item.href} className="text-sm text-muted-foreground transition-colors hover:text-signal">
              {item.label}
            </Link>
          </li>
        ))}
